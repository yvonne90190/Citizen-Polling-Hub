...
bp = Blueprint('comment_and_reply', __name__)


@bp.route('/polls/<path:poll_id>/comments', methods=['GET'])
...
@bp.route('/polls/<path:poll_id>/comments/new', methods=['POST'])
@login_required
def add_comment(poll_id):
    ...
    comment = Comment(user_id=current_user.user_id,
                      content=content, poll_id=poll.poll_id)
...
@bp.route('/polls/<int:poll_id>/comments/<int:comment_id>/replies', methods=['POST'])
@login_required
def add_reply(poll_id, comment_id):
    ...
    reply = Comment(user_id=current_user.user_id, content=content,
                    poll_id=poll.poll_id, parent_id=parent.comment_id)
